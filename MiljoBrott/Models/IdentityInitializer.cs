...
namespace MiljoBrott.Models
{
	...
	{
		private static string coordinatorString = "Coordinator";
		private static string investigatorString = "Investigator";
		private static string managerString = "Manager";
		private static string siteAdminString = "SiteAdmin";
		private static string testPw = "Abc123!";
		...
		private static async Task CreateRoles(RoleManager<IdentityRole> roleM)
		{
			if (!await roleM.RoleExistsAsync(coordinatorString) )
			{
				await roleM.CreateAsync(new IdentityRole(coordinatorString));
			}
			if (!await roleM.RoleExistsAsync(investigatorString)) 
			{
				await roleM.CreateAsync(new IdentityRole(investigatorString));
			}
			if (!await roleM.RoleExistsAsync(managerString)) 
			{
				await roleM.CreateAsync(new IdentityRole(managerString));
			}
			if (!await roleM.RoleExistsAsync(siteAdminString)) 
			{
				await roleM.CreateAsync(new IdentityRole(siteAdminString));
			}
		}

		private static async Task CreateUsers(UserManager<IdentityUser> userM)
		{
			...
			IdentityUser testUser2 = new IdentityUser("Test" + investigatorString);
			await userM.CreateAsync(testUser2, testPw);
			await userM.AddToRoleAsync(testUser2, investigatorString);
			IdentityUser testUser3 = new IdentityUser("Test" + managerString);
			await userM.CreateAsync(testUser3, testPw);
			await userM.AddToRoleAsync(testUser3, managerString);
